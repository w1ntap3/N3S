...
static inline slot_ret_t sort(uint8_t *unordered_entries, const uint8_t *orders,
                              const uint8_t found) {
  if (found == 0)
    return SL_OK;

  for (uint8_t i = 0; i < found; i++) {
    for (uint8_t j = i + 1; j < found; j++) {
      if (orders[i] == orders[j])
        return SL_SAME_ORDER;
    }
  }

  for (uint8_t i = 0; i < found - 1; i++) {
    uint8_t min = i;
    for (uint8_t j = i + 1; j < found; j++) {
      if (orders[j] < orders[min])
        min = j;
    }
    uint8_t tmp = unordered_entries[i];
    unordered_entries[i] = unordered_entries[min];
    unordered_entries[min] = tmp;
  }
  return SL_OK;
}
...
slot_ret_t slot_header_init(void) {
  ...
  for (uint16_t current_slot = 0; current_slot < HEADER_MAX_SLOTS;
       current_slot++) {
    Slot entry;
    eeprom_read(SLOT_ENTRIES_ADDRESS + current_slot * SLOT_ENTRY_SIZE,
...
slot_ret_t slot_occupy(const char *name, const uint8_t name_length,
  ...
  for (uint8_t entry = 0; entry < SLOTS_PER_SECTION; entry++) {
    ...
    eeprom_write(SLOT_ENTRIES_ADDRESS +
                     (section * SLOTS_PER_SECTION + entry) * SLOT_ENTRY_SIZE,
...
slot_ret_t slot_get_by_name(const uint8_t *name, const uint8_t name_len,
  ...
  if (!name)
    return SL_INVALID_NAME;
  if (name_len > SLOT_MAX_FILENAME || name_len < SLOT_MIN_FILENAME)
    return SL_WRONG_NAME_SIZE;
  if (section >= HEADER_SECTIONS)
    return SL_INVALID_SECTION;
  if (!ordered_entries || !entry_count)
    return SL_INVALID_BUFFER;

  uint8_t padded_name[SLOT_MAX_FILENAME];
  memset(padded_name, ' ', SLOT_MAX_FILENAME);
  memcpy(padded_name, name, name_len);

  uint8_t unordered_entries[SLOTS_PER_SECTION];
  uint8_t unordered_orders[SLOTS_PER_SECTION];
  uint8_t found = 0;
  for (uint8_t entry = 0; entry < SLOTS_PER_SECTION; entry++) {
    uint8_t slot_entry[SLOT_ENTRY_SIZE];
    eeprom_read(SLOT_ENTRIES_ADDRESS +
                    SLOTS_PER_SECTION * section * SLOT_ENTRY_SIZE +
                    entry * SLOT_ENTRY_SIZE,
                slot_entry, SLOT_ENTRY_SIZE);
    if (memcmp(slot_entry, padded_name, SLOT_MAX_FILENAME) == 0) {
      unordered_entries[found] = entry;
      unordered_orders[found] = slot_entry[SLOT_MAX_FILENAME];
      found++;
    }
  }

  slot_ret_t ret = sort(unordered_entries, unordered_orders, found);
  if (ret != SL_OK)
    return ret;

  memcpy(ordered_entries, unordered_entries, found);
  *entry_count = found;
  return SL_OK;
}
