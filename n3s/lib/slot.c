...
static inline slot_ret_t sort(uint8_t *unordered_entries, const uint8_t found) {
	return SL_OK;
}
...
slot_ret_t slot_get_by_name(const uint8_t *name, const uint8_t name_len,
	...
	for (uint8_t entry = 0; entry < SLOTS_PER_SECTION; entry++) {
		...
		if (memcmp(slot_entry, name, name_len) == 0) {
			uint8_t empty_slot[SLOT_ENTRY_SIZE] = {0};
			eeprom_write(SLOT_ENTRIES_ADDRESS +
					     SLOTS_PER_SECTION * section *
						     SLOT_ENTRY_SIZE +
	...
	slot_ret_t ret = sort(unordered_entries, found);
	if (ret != SL_OK)
		return ret;
