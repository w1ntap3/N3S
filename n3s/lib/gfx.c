...
void gfx_draw_pixel(gfx_pixel pixel) {
	...
	// Map Y to COM
	uint8_t page =
		(pixel.y - (pixel.y % 8)) / 8; // maps the y to a page 0-7
	uint8_t com_bit =
		(1 << (pixel.y - (page * 8))); // binary 1 in a byte of where
					       // the row is at in that page
...
void gfx_draw_char(gfx_pixel pos, char c, uint8_t invert) {
	...
	for (uint8_t col = 0; col < FONT_WIDTH; col++) {
		...
		uint8_t data = invert ? ~font_array[index + col]
				      : font_array[index + col];
		if (y_offset == 0) {
			oled_buffer[page][seg + col] = data;
		} else {
			oled_buffer[page][seg + col] &=
				~((uint8_t)(0xFF << y_offset));
