...
static inline file_ret_t validate_name(const char *name, const uint8_t name_len,
  ...
  *section = ret;
  *slot_name_len = chars_before_dot;
  return FL_OK;
}
...
file_ret_t file_create(const char *name, const uint8_t name_len) {
  uint8_t section;
  uint8_t slot_name_len;
  file_ret_t file_ret = validate_name(name, name_len, &section, &slot_name_len);
  if (file_ret != FL_OK)
    return file_ret;
  uint8_t pos_in_section;
  slot_ret_t slot_ret =
      slot_occupy(name, slot_name_len, section, &pos_in_section, 0);
...
file_ret_t file_delete(const char *name, const uint8_t name_len) {
  ...
  if (file_ret != FL_OK)
    return file_ret;
  uint8_t ordered_entries[SLOTS_PER_SECTION];
  uint8_t entry_count;
  slot_ret_t slot_ret = slot_get_by_name(
      (uint8_t *)name, slot_name_len, section, ordered_entries, &entry_count);
  if (slot_ret == SL_INVALID_NAME)
    return FL_INVALID_NAME;
  if (slot_ret == SL_WRONG_NAME_SIZE)
    return FL_WRONG_NAME_SIZE;
  if (slot_ret == SL_INVALID_SECTION)
    return FL_INVALID_TYPE;
  if (slot_ret == SL_INVALID_BUFFER)
    return FL_INVALID_BUFFER;
  if (slot_ret == SL_SAME_ORDER)
    return FL_SAME_ORDER_SLOTS;

  for (uint8_t entry = 0; entry < entry_count; entry++) {
    slot_ret = slot_free(section, ordered_entries[entry]);
    if (slot_ret == SL_OK)
      continue;
    if (slot_ret == SL_INVALID_SECTION)
      return FL_UNEXPECTED_ERROR;
    if (slot_ret == SL_ALREADY_FREE)
      return FL_UNEXPECTED_ERROR;
    if (slot_ret == SL_INVALID_SLOT)
      return FL_UNEXPECTED_ERROR;
    if (slot_ret == SL_NOT_INITIALIZED)
      return FL_NOT_INITIALIZED;
  }
